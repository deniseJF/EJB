...
public class CadastroClienteIT {
	...
	public void registrandoClienteSucesso(boolean seguranca, String user, String senha, Clientes cliente, CadastrarCliente cadastro) throws Exception {
		cadastro.setUser(user, senha);
		if (seguranca){
			cadastro.cadastrar(cliente); 
		}else{
			cadastro.cadastrarSemSeguranca(cliente);
		}
	}
    ...
    public void totalClientes() throws Exception {
		try{
			registrandoClienteSucesso(true, "vinicius.vale@4linux.com.br", "123", preparaCliente(1, new Clientes("Vinicius 4Linux2", new Endereco())), cadastroCliente2);
		}catch(EJBAccessException e){
			registrandoClienteSucesso(false, "vinicius.aquino@4linux.com.br", "123", preparaCliente(2, new Clientes("Vinicius 4Linux", new Endereco())), cadastroCliente1);
			totalClientesSingleton(1);
		}catch(Exception e){
