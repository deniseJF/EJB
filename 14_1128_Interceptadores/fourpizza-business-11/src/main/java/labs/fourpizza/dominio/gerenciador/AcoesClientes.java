...
import labs.fourpizza.dominio.interceptadores.AcoesAuditoria;

@Stateless
@LocalBean
//@RolesAllowed({"usuarios"})
@Interceptors(AcoesAuditoria.class)
public class AcoesClientes{// implements GerenciadorClientes{
	...
	public void enviaEmail(Timer timer){
		...
		if(cliente != null ){
			System.out.println("Enviar Email de Aniversário. (TimeOut)");
		}
	}
	
	@Schedule(dayOfWeek="Mon-Fri", hour="3,5")
	public void enviaEmailSchedule(){
		System.out.println("Enviar Email de Aniversário. (Schedule)");
	}
